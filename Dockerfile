# This is an example build stage for the node participant-handler. Here we create the binary in a temporary image.
...
RUN useradd -m -u 1000 -U -s /bin/sh -d /node-dev node-dev && \
  ...
  ln -s /chain-data /node-dev/.local/share/node-participant-handler && \
  # unclutter and minimize the attack surface
  rm -rf /usr/bin /usr/sbin && \
  # check if executable works in this container
  /usr/local/bin/node-participant-handler --version
