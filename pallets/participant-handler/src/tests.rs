use crate::{mock::*, Error, Event, NgoInfo, Category, NgoWaitingList, SellerInfo, SellerWaitingList, NgoActiveList, SellerActiveList};
...
fn test_apply_as_ngo_with_anonymous_identity_returns_identity_error() {
    new_test_ext().execute_with(|| {
        let (ngo, ngo_info) = get_ngo_info();
        assert_noop!(ParticipantHandler::apply_as_ngo(RuntimeOrigin::signed(ngo), ngo_info) , Error::<Test>::IdentityNotFound);
    })
}
...
fn test_apply_as_ngo_with_already_registered_ngo_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        add_identity(ngo);
        assert_noop!(ParticipantHandler::apply_as_ngo(RuntimeOrigin::signed(ngo), ngo_info), Error::<Test>::AlreadyPartOfWaitingList);
    })
}
...
fn test_approve_ngo_with_un_registered_ngo_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(ParticipantHandler::approve_ngo(RuntimeOrigin::signed(0), ngo), Error::<Test>::NotPartOfWaitingList);
    })
}
...
fn test_approve_ngo_with_already_part_of_active_list_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(ParticipantHandler::approve_ngo(RuntimeOrigin::signed(0), ngo), Error::<Test>::AlreadyPartOfActiveList);
    })
}
...
fn test_apply_as_seller_with_anonymous_identity_returns_identity_error() {
    new_test_ext().execute_with(|| {
        let (seller, seller_info) = get_seller_info();
        assert_noop!(ParticipantHandler::apply_as_seller(RuntimeOrigin::signed(seller), seller_info) , Error::<Test>::IdentityNotFound);
    })
}
...
fn test_apply_as_seller_with_already_registered_seller_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        add_identity(seller);
        assert_noop!(ParticipantHandler::apply_as_seller(RuntimeOrigin::signed(seller), seller_info), Error::<Test>::AlreadyPartOfWaitingList);
    })
}
...
fn test_approve_seller_with_un_registered_seller_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(ParticipantHandler::approve_seller(RuntimeOrigin::signed(0), seller), Error::<Test>::NotPartOfWaitingList);
    })
}
...
fn test_approve_seller_with_already_part_of_active_list_returns_error() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(ParticipantHandler::approve_seller(RuntimeOrigin::signed(0), seller), Error::<Test>::AlreadyPartOfActiveList);
    })
}
