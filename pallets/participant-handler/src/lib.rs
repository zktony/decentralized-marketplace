...
pub mod pallet {
	...
	#[derive(Encode, Decode, Clone, Copy, Debug, MaxEncodedLen, TypeInfo, PartialOrd, PartialEq, Eq)]
	pub enum Category {
		Pharmaceutical = 0,
		Stationery = 1,
		Grocery = 2,
	...
	#[derive(Encode, Decode, Clone, Debug, MaxEncodedLen, TypeInfo, Default, PartialOrd, PartialEq)]
	...
	pub type NgoWaitingList<T:Config> = StorageMap<_, Blake2_128Concat, T::AccountId, NgoInfo<T::Hash>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn get_ngo_active_list)]
	pub type NgoActiveList<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, NgoInfo<T::Hash>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn get_seller_waiting_list)]
	pub type SellerWaitingList<T:Config> = StorageMap<_, Blake2_128Concat, T::AccountId, SellerInfo<T::Hash>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn get_seller_active_list)]
	pub type SellerActiveList<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, SellerInfo<T::Hash>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(2)]
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
		pub fn apply_as_seller(origin: OriginFor<T>, seller_info: SellerInfo<T::Hash>) -> DispatchResult {
			let who = ensure_signed(origin)?;
			ensure!(
				pallet_identity::Pallet::<T>::has_identity(&who, 3),
				Error::<T>::IdentityNotFound
	...
	impl<T: Config> Pallet<T> {
		fn do_apply_as_ngo(applicant: &T::AccountId, ngo_info: NgoInfo<T::Hash>) -> DispatchResult {
			ensure!(!<NgoWaitingList<T>>::contains_key(applicant), Error::<T>::AlreadyPartOfWaitingList);
			ensure!(!<NgoActiveList<T>>::contains_key(applicant), Error::<T>::AlreadyPartOfActiveList);
			<T as Config>::Currency::reserve(applicant, T::NgoStakingAmount::get().saturated_into())?;
		...
		fn do_apply_as_seller(applicant: &T::AccountId, seller_info: SellerInfo<T::Hash>) -> DispatchResult {
			ensure!(!<SellerWaitingList<T>>::contains_key(applicant), Error::<T>::AlreadyPartOfWaitingList);
			ensure!(!<SellerActiveList<T>>::contains_key(applicant), Error::<T>::AlreadyPartOfActiveList);
			<T as Config>::Currency::reserve(applicant, T::SellerStakingAmount::get().saturated_into())?;
			<SellerWaitingList<T>>::insert(applicant, seller_info);
			Ok(())
		}

		fn do_approve_seller(applicant: &T::AccountId) -> DispatchResult {
			ensure!(<SellerWaitingList<T>>::contains_key(applicant), Error::<T>::NotPartOfWaitingList);
			ensure!(!<SellerActiveList<T>>::contains_key(applicant), Error::<T>::AlreadyPartOfActiveList);
